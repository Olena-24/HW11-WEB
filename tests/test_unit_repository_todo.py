...
class TestAsyncTodo(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_get_all_todos(self):
        limit = 10
        offset = 0
        todos = [Contact(id=1, title='test_title_1', description='test_description_1', user=self.user),
                 Contact(id=2, title='test_title_2', description='test_description_2', user=self.user)]
    ...
    async def test_update_todo(self):
        body = ContactUpdateSchema(title='test_title', description='test_description', completed=True)
        mocked_todo = MagicMock()
        mocked_todo.scalar_one_or_none.return_value = Contact(id=1, title='test_title', description='test_description',
                                                           user=self.user)
